...
#### 데이터 최신화 (sp-refresh, 로컬 전용)

저장소엔 2026-06 시점 데이터 스냅샷이 포함돼 바로 쓸 수 있다. **최신 시세로 갱신**하려면
(로컬 PC에서, 인터넷 필요 — yfinance 사용):
```bash
sp-refresh            # 시세(KR+US) + 거시(지수·환율) 갱신  ★권장(매일 변하는 데이터)
sp-refresh all        # + 섹터 + 미국재무
```
한국 재무(DART)·검색관심도(Naver)는 분기성이라 자주 갱신 불필요(기존 스냅샷 유지, 키 필요).
※ 클라우드(웹) 세션은 Yahoo 차단으로 `sp-refresh` 실행 불가 — 로컬에서만 동작.
