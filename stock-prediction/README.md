# stock-prediction — 주식 수익률 예측 알고리즘

한국(KRX: KOSPI/KOSDAQ)·미국 시장 종목의 **가격·재무·심리 데이터**를 수집해
피처를 만들고, 머신러닝/딥러닝 모델로 단기~중기 **방향성·수익률을 예측**하는 연구·실험 파이프라인.
...
| ★ 최종결합 | ✅ 완료 | `sp-final` / [결과](docs/capstone-results.md): **분기+시장중립+레짐필터 누적 → 샤프 -0.04→1.57, MDD -40%→-3%** |
| 4. 심리 | ⬜ 예정 | 뉴스/검색 감성 추가 |

### 핵심 결과 (누적 ablation, h=20 롱숏 LightGBM)

| 구성 | 샤프 | MDD |
|---|---:|---:|
| C0 원조(풀링+연간) | -0.04 | -40.3% |
| C1 +분기 펀더멘털 | 0.92 | -33.4% |
| C2 +시장중립 | 1.23 | -11.1% |
| C3 +레짐필터(최종) | **1.57** | **-2.7%** |

> ⚠️ 표본이 작아(리밸 ~19회) 수치의 *크기*는 미확정 — 각 개선의 *방향성* 증거로 해석.
